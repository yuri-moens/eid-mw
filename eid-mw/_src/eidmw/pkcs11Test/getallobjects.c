			...
			if (ReturnedSuccesfull(frv,&(retVal.pkcs11rv), "C_GetSlotList", "test_getallobjects" ))
			{
				...
				if(slotIds != NULL)
				{
					...
					if (ReturnedSuccesfull(frv,&(retVal.pkcs11rv), "C_GetSlotList (X2)", "test_getallobjects" ))
					{
						...
						pLabel = malloc (256);
						ullabelLen = 255;//last one is for the string termination

						if(pLabel != NULL)
						{
							pValue = (CK_UTF8CHAR_PTR)malloc (4096);
							ulvalueLen = 4095;
							if(pValue != NULL)
							{
							...
							{
								retVal.pkcs11rv = CKR_HOST_MEMORY;
							}
							free(pLabel);
						}
						else
...
testRet test_getallidobjects() {
	...
	if (InitializeTest(&handle,&functions))
	{
		...
		{	
			...
			for ( i=0;i< 3; i++)
			{
				testlog(LVL_INFO,"press enter to read card\n");
				getch();
				frv = (*functions->C_GetSlotList) (CK_TRUE, 0, &slot_count);
				if (ReturnedSuccesfull(frv,&(retVal.pkcs11rv), "C_GetSlotList", "test_getallobjects" ))
				{
