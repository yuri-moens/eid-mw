				...
				if(slotIds != NULL)
				{
					...
					if (ReturnedSuccesfull(frv,&(retVal.pkcs11rv), "C_GetSlotList (X2)", "test_sign" ))
					{
						...
						for (slotIdx = 0; slotIdx < slot_count; slotIdx++) 
						{
							...
							CK_BYTE signature[258];
							CK_ULONG signLength = 258;
							CK_BYTE signatureMultiPart[258];
							CK_ULONG signMultiPartLength = 258;
...
testRet test_sign(void)
{
	...
	//testret = test_sign_allmechs();
	
	testret = test_sign_mech(CKM_RSA_PKCS);//CKM_SHA1_RSA_PKCS_PSS CKM_SHA384_RSA_PKCS);
