...
testRet test_logslotevents(CK_ULONG nrOfEvents,CK_FLAGS flags) {
	...
	{
		for (ulEventCounter = 0;ulEventCounter < nrOfEvents;ulEventCounter++)
		{
			frv = (*functions->C_WaitForSlotEvent)(flags, &slotId,NULL_PTR);
			testlog(LVL_INFO, "C_WaitForSlotEvent returned\n");
			if (frv != CKR_NO_EVENT)
			{
...
testRet test_logslotevents_userinteraction_flags(CK_FLAGS flags) {
	void *handle = NULL;				//handle to the pkcs11 library
	CK_FUNCTION_LIST_PTR functions;		// list of the pkcs11 function pointers
...
testRet test_logslotevents_userinteraction()
{
	testRet retVal = {CKR_OK,TEST_PASSED};	//return values of this test
	retVal = test_logslotevents_userinteraction_flags(CKF_DONT_BLOCK);
	if((retVal.pkcs11rv == CKR_OK) && (retVal.basetestrv == TEST_PASSED)){
		retVal = test_logslotevents_userinteraction_flags(0);
	}
	return retVal;
}

DWORD WINAPI test_logslotevents_finalize( LPVOID thefunctions ) 
{
...
testRet test_logslotevents_whilefinalize() {
	void *handle = NULL;				//handle to the pkcs11 library
	CK_FUNCTION_LIST_PTR functions;		//list of the pkcs11 function pointers
	...
	if((retVal.pkcs11rv == CKR_OK) && (retVal.basetestrv == TEST_PASSED))
	{
		...
		hThreadHandle = CreateThread( 
			...
			test_logslotevents_finalize,			// thread function name
...
testRet test_logslotevents_nonblocking(void) {
	return test_logslotevents(10,CKF_DONT_BLOCK);
}

testRet test_logslotevents_blocking(void) {
	return test_logslotevents(10,0);
}